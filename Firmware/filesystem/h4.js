...
function ajax(url, decode = true) {
  ...
  http.addEventListener("load", function (e) {
    ...
    if (decode) {
      ...
      if (j.res) {
        toaster("Error: " + j.res + " " + j.msg);
      } else {
        j.lines.forEach(function (l) {
          r.innerHTML += l + "\n";
        });
        document
          .querySelectorAll("input.edit")
          .forEach((x) => x.classList.remove("edit"));
      }
    }
  });
...
function ajaxvalue(e) {
  ajax("h4/config/" + e.target.id + "=`" + e.target.value + "`");
}

function btnImg(n, v) {
  n.src = "/" + n.id + parseInt(v) + ".jpg";
}
...
function makeValue(id, type, active, color, value) {
  ...
  switch (type) {
    case 0: // TEXT
      valueNode = makeBaseValueNode(
        id,
        "div",
        "uv",
        (n, e) => (n.innerHTML = e.data)
      );
    ...
    case 2: // BOOL
      ...
      break;
    case 3: // input
      valueNode = makeBaseValueNode(id, "input", "", (n, e) => {
        n.value = e.data;
      });
      valueNode.type = "text";
      valueNode.value = value;
      valueNode.addEventListener("focus", (e) =>
    ...
    case 4: // image button
      ...
      break;
    case 5: // dropdown
      valueNode = makeBaseValueNode(id, "select", "", (n, e) =>
        options(n, e.data)
      );
    ...
    case 6: // image
      ...
      source.addEventListener(
        id,
        (e) => (document.getElementById(id).src = e.data)
      );
      break;
    default:
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  source.onerror = function (e) {
    document
      .querySelectorAll("div.hanger")
      .forEach((d) => (d.style.background = "#ff0000"));
  };

  toaster(
    "Thank you for using H4/Plugins - please support me on Patreon, see link below"
  );
  ...
  document.getElementById("qh").addEventListener(
    "click",
    function (e) {
      ajax("help", true);
    },
