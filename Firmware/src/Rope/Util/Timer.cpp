...
void Timer::waitFor(unsigned long interval, const Timer::UnitOfTime unit) const
{
    ...
    unsigned long totalTimeElapsed = timeElapsed(UnitOfTime::Microseconds);

    if (totalTimeElapsed < interval)
    {
        unsigned long timeRemaining = interval - totalTimeElapsed;
...
unsigned long Timer::timeElapsed(const Timer::UnitOfTime unit) const
{
    ...
    long timeElapsed = micros() - startTime;
